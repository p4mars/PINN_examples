...
selected_folders = ["Room", "Outside"]
train_group_count = 1  # Number of groups used for training in each split
...
for i, (train_folders, validation_folders) in enumerate(splits):
    ...
    PINN_cooling_down_experiment = NN(7, 1, 64, 4, epochs=50000, batch_size=64, use_batch=False, lr=1e-5, loss_terms=loss_functions, learn_params=None, unique_input=True, show_plots=False)
    ...
    # I-PINN
    # Create network (re-init weights and biases)
    IPINN_cooling_down_experiment = NN(7, 1, 64, 4, epochs=50000, batch_size=64, use_batch=False, lr=1e-5, loss_terms=loss_functions_learn, learn_params=learn_parameters, unique_input=True, show_plots=False)
    ...
    # Data driven
    # Create network (re-init weights and biases)
    NN_cooling_down_experiment = NN(7, 1, 64, 4, epochs=50000, batch_size=64, use_batch=False, lr=1e-5, show_plots=False)
    ...
    for val_file in validation_files:
        ...
        # Obtain errors PINN
        _ , _, _, error_average_pinn, error_max_pinn, execution_time_pinn = prep_data_cooling_surrogate(validation_folder, scaled=False, n_outputs=1, n_samples=1e6, start_time=None, end_time=None).compare_with_data(PINN_cooling_down_experiment.predict)
        
        # Obtain errors IPINN
        _ , _, _, error_average_ipinn, error_max_ipinn, execution_time_ipinn = prep_data_cooling_surrogate(validation_folder, scaled=False, n_outputs=1, n_samples=1e6, start_time=None, end_time=None).compare_with_data(IPINN_cooling_down_experiment.predict)
        
        # Obtain errors Data-driven
        _ , _, _, error_average_data_driven, error_max_data_driven, execution_time_data_driven = prep_data_cooling_surrogate(validation_folder, scaled=False, n_outputs=1, n_samples=1e6, start_time=None, end_time=None).compare_with_data(NN_cooling_down_experiment.predict)
...
if save_data == 1:
    ...
    output_csv_path = "/Users/tristanhirs/Downloads/Thesis/Measurements_cup/Measurements/Error_file/surrogate_room_outside_40_points_cooling.csv"  # Specify the path where you want to save the CSV
