...
class NN(nn.Module):
    ...
    def __init__(self, N_INPUT, N_OUTPUT, N_HIDDEN, N_LAYERS, activation_fn=nn.ReLU, init_method=nn.init.xavier_uniform_, optimizer_type=optim.Adam, batch_size=32, use_batch=False, epochs=1000, lr=1e-3, loss_init=nn.MSELoss(), loss_terms=None, learn_params=None, unique_input=False, show_plots=True, dropout_prob=0, scheduler_step_size=1, scheduler_gamma=1):
        ...
        init_method : nn.<init_method>, optional
            Type of weight initialization method. The default is nn.init.xavier_uniform_. To be able to change this parameter, torch.nn should be imported.
        optimizer_type: optim.<optimizer_type>, optional
            Type of optimizer used during the training process. The default is optim.Adam. To be able to change this parameter, torch.optim should be imported.
        ...
        epochs : int, optional
            Amount of epochs used during training. The default value is set to 1000.
        lr : float, optional
            Learning rate. The default value is set to 1e-3.
        ...
        loss_terms : Dictionary, optional
            Insert additional loss terms such as physics and boundary loss. The default is None. The data loss terms are standard embedded inside the class.
        ...
        scheduler_gamma: Float, optional
            Defines how much the learning rate is decreased each step size by the scheduler. This works as follows: learning rate (i+1) = learning rate (i) * gamma. The standard is set to 1, meaning that there is no decrease in learning rate during the training process.
        Returns
        -------
        None.
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        ...
        scheduler = StepLR(optimizer, step_size=self.scheduler_step_size, gamma=self.scheduler_gamma)  # change learning rate by half each 1000 steps
